<?php
	...
	$mass = $quantity * $unit_size;
	$calories = round($data["calories"] * $quantity * $unit_size / 100, 0);
	$protein = round($data["protein"] * $quantity * $unit_size / 100, 0);
	$carbs = round($data["carbs"] * $quantity * $unit_size / 100, 0);
	$fat = round($data["fat"] * $quantity * $unit_size / 100, 0);
	$unit = $data["unit"];

	$sql = "INSERT INTO consumption (id, name, mass, meal, calories, protein, carbs, fat, unit) VALUES ('id', '$name', '$mass', '$meal', '$calories', '$protein', '$carbs', '$fat', '$unit')";
